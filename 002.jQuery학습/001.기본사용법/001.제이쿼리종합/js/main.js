    ...
    btns.first().click((e) => { // e 이벤트 전달변수
      ...
      .next().click((e) => {
        ...
        let callFn = () => { // 애니후 실행함수
            ...
            .fadeIn(200,()=>{
                ...
                let tg = bd.eq(7);
                // 2-1. 좀비 윗층 올라오기
                // 같은 대상에 연속해서 순서대로 실행됨
                // 큐(Queue)에 등록되어 순서대로 실행
                tg.find('.mz')
                .animate({ // 아래에서 위로 올라오기
                    bottom: '100%'
                }, 300,"easeOutBack")
                .animate({ //오른쪽에서 달려오기
                    right: '120%'
                ...
                () => { // 애니후 실행함수
                    ...
                    setTimeout(()=>{
                        ...
                        // attr(속성명,속성값) -> 속성값 변경 매서드
                        // 비교) JS의  setAttribute() 
                        // 참고) 속성값 가져오기는 -> attr(속성명)
                        // 비교) JS의 gatAttribute()
      ...
      .next().click((e) => {
        ...
        let callFn = () => { // 애니후 실행함수
            ...
            // 1.주사기 돌리기 
            // 주의: tranfrom은 animate에서 사용 불가!
            // tranform은 css로 transition으로 구현!
            $('.inj').css({
